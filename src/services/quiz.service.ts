...
export class QuizService {
  ...
  getQuizById(quizId: number): Observable<IQuizState> {
    return this.http.get<IQuizState>(`http://localhost:5000/quiz/start/${quizId}`)
  }
  
  getQuestions(quizId: number): Observable<IQuestion[]> {
    return this.http.get<IQuestion[]>(`http://localhost:5000/quiz/questions/${quizId}`)
  }
  
  checkAnswer(questionId: number, answerId: number): Observable<{is_correct: boolean}> {
    return this.http.get<{is_correct: boolean}>(`http://localhost:5000/quiz/answer_check?answer_id=${answerId}&question_id=${questionId}`)
  }
}
